let handler = async (m, { conn, usedPrefix, command, text }) => {
	...
	if (!text && !m.quoted) return conn.reply(m.chat, `welche Nummer?\nBeispiel: *${usedPrefix}${command} ${global.owner[0]}*\n@markiere/reagier auf benutzer`, m)
	//let exists = await conn.isOnWhatsApp(number)
	// if (exists) return conn.reply(m.chat, `*Nomor target tidak terdaftar di WhatsApp*`, m)
	if (isNaN(number)) return conn.reply(m.chat, `Die von ihnen eingegebene Nummer ist ungültig!`, m)
	if (number.length > 15) return conn.reply(m.chat, `Die von ihnen eingegebene Nummer ist ungültig!`, m)
	try {
		if (text) {
		...
		let anu = `@${number} erfolgreich aus *DATABASE* entfernt`
